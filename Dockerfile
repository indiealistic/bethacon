...
WORKDIR /go/src/github.com/indiealistic/bethacon/
...
RUN CGO_CFLAGS_ALLOW="-D__BLST_PORTABLE__" CGO_CFLAGS="-D__BLST_PORTABLE__" CGO_ENABLED=1 GOOS=linux go install -a -tags blst_enabled -ldflags "-linkmode external -extldflags \"-static -lm  -msoft-float -Wl,-y,__sigsetjmp_aux\"" ./cmd/bethacon
...
COPY --from=builder /go/bin/bethacon /go/bin/bethacon

# This is needed for healthcheck
EXPOSE 5678

ENTRYPOINT ["/go/bin/bethacon"]
